...
int main(int argc, char ** argv)
{
  ...
  for (int r = size; r > 1; r=r/2) {
    if (rank < r / 2) { // Je recois le tableau de mon partenaire puis je le fusionne avec le mien
      ...
      count *= 2;
      std::swap(A, tempMerge); // le tableau fusionne devient mon tableau pour l'etape suivante
    } else if (rank >= r / 2 && rank < r) { // J'envoie mon tableau a mon partenaire 
      int partner = rank-r/2;
